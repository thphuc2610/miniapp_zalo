...
import { checkoutState, datlichFormState } from "features/datlich/state/booking.state";
...
export const DatlichPage: FC = () => {
  ...
  const [formState, setFormState] = useRecoilState(datlichFormState);
  const { selectedServices, selectedKTVs, guestCount, selectedDate, selectedTime } = formState;

  const setSelectedServices = (val: string[] | ((prev: string[]) => string[])) => {
    setFormState(prev => ({ ...prev, selectedServices: typeof val === 'function' ? val(prev.selectedServices) : val }));
  };
  const setSelectedKTVs = (val: string[] | ((prev: string[]) => string[])) => {
    setFormState(prev => ({ ...prev, selectedKTVs: typeof val === 'function' ? val(prev.selectedKTVs) : val }));
  };
  const setGuestCount = (val: number | ((prev: number) => number)) => {
    setFormState(prev => ({ ...prev, guestCount: typeof val === 'function' ? val(prev.guestCount) : val }));
  };
  const setSelectedDate = (val: string | ((prev: string) => string)) => {
    setFormState(prev => ({ ...prev, selectedDate: typeof val === 'function' ? val(prev.selectedDate) : val }));
  };
  const setSelectedTime = (val: string | undefined | ((prev: string | undefined) => string | undefined)) => {
    setFormState(prev => ({ ...prev, selectedTime: typeof val === 'function' ? val(prev.selectedTime) : val }));
  };
  ...
  useEffect(() => {
    ...
    getBusyTechnicianIds({
      ...
      const validKTVs = selectedKTVs.filter(id => !ids.includes(String(id)));
      if (validKTVs.length !== selectedKTVs.length) {
        setSelectedKTVs(validKTVs);
      }
    });
  ...
  }, [selectedBranch?.id, selectedDate, selectedKTVs, selectedServices, selectedTime]);
  ...
  useEffect(() => {
    if (!selectedBranch?.id || !selectedDate || selectedKTVs.length === 0 || selectedServices.length === 0) {
      setBusyTimeSlots([]);
      return;
    }
    ...
    Promise.all(TIME_SLOTS.map(async (slot) => {
      ...
      return selectedKTVs.some(ktvId => ids.includes(String(ktvId))) ? slot : null;
    })).then((slots) => {
      if (cancelled) return;
      const busySlots = slots.filter(Boolean) as string[];
      setBusyTimeSlots(busySlots);
      if (selectedTime && busySlots.includes(selectedTime)) {
  ...
  const handleConfirm = async () => {
    if (!selectedDate || !selectedTime) {
      openSnackbar({ text: "Vui lòng chọn ngày và giờ hẹn", type: "error" });
      return;
    }
    ...
    if (selectedKTVs.length > 0) {
      setCheckingAvailability(true);
      try {
        const busySelected = selectedKTVs.filter(ktvId => busyTechnicianIds.includes(String(ktvId)));
        if (busySelected.length > 0) {
          setSelectedKTVs(selectedKTVs.filter(ktvId => !busyTechnicianIds.includes(String(ktvId))));
          openSnackbar({
            text: "Kỹ thuật viên đã chọn có lịch bận trong khung giờ này. Vui lòng chọn kỹ thuật viên hoặc giờ khác.",
            type: "error",
          });
          return;
        }
    ...
    const technicianIds = selectedKTVs.join(",");
    const technicianName = selectedKTVs.map(id => technicians.find(t => String(t.id) === String(id))?.fullName).filter(Boolean).join(", ");
    ...
    selectedServices.forEach(svcId => {
      ...
      checkoutItems.push({
        ...
        time: selectedTime,
        quantity: guestCount,
        technicianId: technicianIds || null,
    ...
    <Page className="page" style={{ paddingBottom: 140, background: "#fdf2f8" }}>
      ...
      <Box p={4} style={{ display: "flex", flexDirection: "column", gap: 24, minHeight: "100vh" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#fff", borderRadius: 12, border: "1px solid #fce7f3", padding: "6px", boxShadow: "0 2px 12px rgba(131, 24, 67, 0.02)", width: "100%", boxSizing: "border-box" }}>
            <button onClick={() => {
              ...
              if (selectedKTVs.length > newCount) {
                setSelectedKTVs(selectedKTVs.slice(0, newCount));
              }
            }} style={{ width: 34, height: 34, borderRadius: "8px", border: "none", background: "#fdf2f8", color: "#be185d", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div>
          ...
          {guestCount > 1 && selectedServices.length > 1 && (
            <div style={{ marginTop: 8, fontSize: 11.5, color: "#d97706", background: "#fef3c7", padding: "8px 12px", borderRadius: 8, fontStyle: "italic", lineHeight: 1.4 }}>
              *Lưu ý: Mỗi người sẽ sử dụng toàn bộ dịch vụ đã chọn như nhau.
            </div>
          )}
        </div>
        ...
        <div style={{ order: 4 }}>
          ...
          <Select
            placeholder={guestCount > 1 ? `Chọn tối đa ${guestCount} KTV` : "Chọn kỹ thuật viên"}
            value={selectedKTVs}
            multiple={true}
            onChange={(val) => {
              const arr = Array.isArray(val) ? val.map(String) : [String(val)];
              if (arr.length > guestCount) {
                openSnackbar({ text: `Chỉ được chọn tối đa ${guestCount} kỹ thuật viên`, type: "warning" });
                return;
              }
              setSelectedKTVs(arr);
            }}
          ...
          <div style={{ background: "#fff", border: "1px solid #fce7f3", borderRadius: 16, padding: "12px 10px", boxShadow: "0 2px 12px rgba(131, 24, 67, 0.02)" }}>
            <div style={{
              ...
              {TIME_SLOTS.map((slot) => {
                ...
                const isBusyTime = selectedKTVs.length > 0 ? busyTimeSlots.includes(slot) : false;
