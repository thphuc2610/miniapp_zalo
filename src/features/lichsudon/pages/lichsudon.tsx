import { bookingHistoryState, quickBookingState } from "features/datlich/state/booking.state";
import React, { FC, useCallback, useEffect, useState } from "react";
import { Header, Modal, Page, Sheet, useSnackbar } from "zmp-ui";
import { useRecoilState, useSetRecoilState } from "recoil";
import { type BookingHistoryItem } from "features/datlich/state/booking.state";
...
const getStatusFilterBackground = (status: string) => {
  const color = getStatusColor(status);
  if (status === STATUS_LIST[0]) return "#fdf2f8";
  if (color === "#d97706") return "#fffbeb";
  if (color === "#2563eb") return "#eff6ff";
  if (color === "#8b5cf6") return "#f5f3ff";
  if (color === "#10b981") return "#ecfdf5";
...
export const OrderHistoryPage: FC = () => {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      const mapped: BookingHistoryItem[] = (bookings || []).map((booking: any) => {
        ...
        const quantity = booking.quantity || booking.guestCount || 1;
  ...
  return (
    <Page style={{ height: "100vh", background: "#fdf2f8", overflowY: "auto", overflowX: "hidden", paddingBottom: 80 }}>
      ...
      <div style={{ background: "rgba(255, 247, 251, 0.96)", borderBottom: "1px solid var(--filter-border)", display: "flex", overflowX: "auto", padding: "12px 16px", gap: 8, scrollbarWidth: "none", msOverflowStyle: "none", WebkitOverflowScrolling: "touch", boxShadow: "var(--shadow-chip)" }}>
        ...
        {STATUS_LIST.map((status) => {
          ...
          return (
            <button
              ...
              style={{
                border: isSelected ? `1px solid ${statusColor}` : "1px solid var(--filter-border)",
                background: isSelected ? statusColor : getStatusFilterBackground(status),
          ...
          filtered.map((order: any) => {
            ...
            return (
              <div
                ...
                style={{ background: "#fff", borderRadius: 12, padding: 10, display: "flex", gap: 12, boxShadow: "var(--shadow-card)", border: "1px solid rgba(190, 24, 93, 0.16)", alignItems: "center", cursor: "pointer" }}
              >
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 2 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      {order.quantity > 1 && (
                        <span style={{ fontSize: 11, color: "#be185d", fontWeight: 700, padding: "2px 6px", background: "#fdf2f8", borderRadius: 6 }}>
                          x{order.quantity} khách
                        </span>
                      )}
                    </div>
                    <div style={{ display: "flex", gap: 8 }}>
                      {canCancel && (
                        <button
                          ...
                          style={{ border: "1px solid rgba(190, 24, 93, 0.18)", background: "#fff", color: "#4b5563", padding: "5px 12px", borderRadius: 999, fontSize: 11, fontWeight: 800, cursor: cancellingId === order.id ? "wait" : "pointer", opacity: cancellingId === order.id ? 0.65 : 1 , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}
                        >
                      ...
                      {(order.status === "Đã hoàn thành" || order.status === "Đã hủy") && (
                        <button
                          ...
                          style={{ border: "1px solid rgba(190, 24, 93, 0.18)", background: "#fff", color: "#be185d", padding: "5px 12px", borderRadius: 999, fontSize: 11, fontWeight: 800, cursor: "pointer" , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}
                        >
                          Đặt lại
                        </button>
                      )}
                      {order.status === "Đã hoàn thành" && (
                        hasFeedback ? (
                          <button
                            disabled
                            style={{ border: "1px solid rgba(190, 24, 93, 0.18)", background: "#f9fafb", color: "#6b7280", padding: "5px 12px", borderRadius: 999, fontSize: 11, fontWeight: 800, cursor: "not-allowed" , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}
                          >
                            Đã đánh giá
                          </button>
                        ) : (
                          <button
                            ...
                            style={{ border: "1px solid rgba(190, 24, 93, 0.18)", background: "#fffbeb", color: "#b45309", padding: "5px 12px", borderRadius: 999, fontSize: 11, fontWeight: 800, cursor: "pointer" , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}
                          >
                            Đánh giá
                          </button>
                        )
      ...
      <Sheet visible={Boolean(selectedDetail)} onClose={() => setSelectedDetail(null)} autoHeight handler>
        {selectedDetail ? (() => {
          ...
          <div style={{ padding: "20px 18px 28px", background: "#fff", borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#831843", marginBottom: 16, textAlign: "center" }}>Chi tiết lịch hẹn</div>
            {[
              ["Dịch vụ", detail.title ? <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>{String(detail.title).split(",").map((s, i) => <div key={i}>{s.trim()}</div>)}</div> : "-"],
              ["Số lượng khách", detail.quantity && detail.quantity > 1 ? `${detail.quantity} người` : "1 người"],
              ["Kỹ thuật viên", detail.technicianName ? <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>{String(detail.technicianName).split(",").map((s, i) => <div key={i}>{s.trim()}</div>)}</div> : "Chưa gán"],
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button
                ...
                style={{ flex: 1, height: 44, borderRadius: 10, border: "1px solid rgba(190, 24, 93, 0.18)", background: "#fff", color: "#831843", fontSize: 14, fontWeight: 800 , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}
              >
                Giữ lịch
              </button>
              <button
      ...
      >
        {feedbackTarget ? (
          <div>
            ...
            <div style={{ display: "flex", gap: 12, padding: 10, border: "1px solid rgba(190, 24, 93, 0.18)", borderRadius: 14, background: "#fdf2f8", marginBottom: 16 , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}>
            ...
            <textarea
              ...
              rows={4}
              style={{ width: "100%", boxSizing: "border-box", border: "1px solid rgba(190, 24, 93, 0.18)", borderRadius: 12, padding: "10px 12px", fontSize: 13, fontWeight: 600, outline: "none", resize: "none", fontFamily: "inherit", marginBottom: 16 , boxShadow: "0 6px 8px rgba(0, 0, 0, 0.14)"}}
            />
            <div style={{ display: "flex", gap: 10 }}>
              <button
