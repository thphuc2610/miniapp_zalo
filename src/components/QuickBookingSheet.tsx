...
import { quickBookingState, cartState, checkoutState, selectedBranchState, selectedUserLoginToken, loginPromptState } from "state";
import { getBusyTechnicianIds, getSpaTechnicians, SpaTechnician } from "service/spaData";
...
export const QuickBookingSheet: FC = () => {   
  ...
  useEffect(() => {
    if (!state.isOpen || !state.item || !activeBranch?.id || !selectedDate || !selectedKTV) {
      setBusyTimeSlots([]);
      return;
    }
