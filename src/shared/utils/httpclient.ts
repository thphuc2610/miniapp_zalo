import { selectedUserLoginToken, refreshTokenState } from "features/xacthuc/state/auth.state";
import axios, { Method, AxiosResponse, AxiosError } from "axios";
import { getRecoil, setRecoil } from "recoil-nexus"; 

import { clearStoredSession, isAccessTokenUsable, saveStoredSession } from "./authSession";
import { API_ENDPOINTS, buildApiUrl, getApiBaseUrl } from "utils/api";
...
axiosInstance.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (
      ...
      originalRequest?.url?.includes(API_ENDPOINTS.identity.refreshToken)
    ) {
    ...
    try {
      ...
      saveStoredSession(newAccessToken, newRefreshToken); 
