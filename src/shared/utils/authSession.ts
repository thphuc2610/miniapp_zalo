...
export const saveStoredSession = (accessToken: string, refreshToken: string) => {
  ...
  localStorage.setItem(AUTH_CONFIRMED_KEY, "1");
...
export const getStoredAccessToken = () => {
  ...
  memoryAccessToken = sessionStorage.getItem(ACCESS_TOKEN_KEY) || localStorage.getItem(ACCESS_TOKEN_KEY);
...
export const getStoredRefreshToken = () => {
  ...
  if (typeof window === "undefined") return null;
  memoryRefreshToken = sessionStorage.getItem(REFRESH_TOKEN_KEY) || localStorage.getItem(REFRESH_TOKEN_KEY);
